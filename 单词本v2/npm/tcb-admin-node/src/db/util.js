...
Util.encodeTimestamp = (stamp => {
    ...
    return {
        $timestamp: Math.floor(stamp.getTime() / 1e3)
    };
});
...
Util.encodeDocumentDataForReq = ((document, merge = false, concatKey = true) => {
    ...
    keys.forEach(key => {
        ...
        if (type === constant_1.FieldType.GeoPoint) {
            realValue = {
                [key]: Util.encodeGeoPoint(item).coordinates
            };
        } else if (type === constant_1.FieldType.Timestamp) {
            realValue = {
                [key]: Util.encodeTimestamp(item)
            };
        } else if (type === constant_1.FieldType.ServerDate) {
            realValue = {
                [key]: Util.encodeServerDate(item)
            };
        } else if (type === constant_1.FieldType.Object) {
            if (concatKey) {
                realValue = getCommandVal({
                    [key]: item
                });
            } else {
                realValue = {
                    [key]: Util.encodeDocumentDataForReq(item, merge, concatKey)
                };
            }
